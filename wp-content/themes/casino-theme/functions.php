...
function casino_register_post_type() {
...
add_action('init', 'casino_register_post_type');

function casino_register_taxonomies() {
...
add_action('init', 'casino_register_taxonomies');

function casino_register_meta_boxes() {
    add_meta_box(
        'casino_details',
        'Данные казино',
        'casino_render_details_metabox',
        'casino',
        'normal',
        'high'
    );
    ...
    add_meta_box(
        'casino_bonus',
        'Бонусы',
        'casino_render_bonus_metabox',
        'casino',
        'normal',
        'default'
    );
}
add_action('add_meta_boxes', 'casino_register_meta_boxes');

function casino_render_details_metabox($post) {
    wp_nonce_field('casino_save_meta', 'casino_meta_nonce');
...
function casino_render_bonus_metabox($post) {
...
function casino_save_meta($post_id) {
    if (!isset($_POST['casino_meta_nonce']) || !wp_verify_nonce($_POST['casino_meta_nonce'], 'casino_save_meta')) {
        return;
    }
...
add_action('save_post_casino', 'casino_save_meta');

function casino_filter_archive_query($query) {
...
add_action('pre_get_posts', 'casino_filter_archive_query');
